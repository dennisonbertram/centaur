...
@pytest.mark.asyncio
async def test_worker_fails_empty_turn_done_after_stream_error(db_pool):
    from api.runtime_control import _process_execution
    ...
    await db_pool.execute(
        ...
        "execution_id, thread_key, assignment_generation, execute_id, request_hash, status, "
        "delivery, metadata, hard_deadline_at"
        ") VALUES ($1, $2, 1, 'exec-compact', 'hash-compact', 'running', '{}'::jsonb, '{}'::jsonb, NOW() + INTERVAL '10 minutes')",
        execution_id,
        thread_key,
    )
    await db_pool.execute(
    ...
    assert "remote compact task" in (execution["error_text"] or "")
